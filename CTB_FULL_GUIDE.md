...
### скрипт авто-проверки и патчинга
...
nano avto_solver.sh
# Вставьте код
chmod +x avto_solver.sh

./avto_solver.sh

Скрипт:
Запросит имя задачи
URL репозитория (опционально)
URL веб-приложения для ZAP (опционально)
Автоматически запустит все проверки
Создаст отчеты в ./ctf_reports/<TASK_NAME>/
Сгенерирует рекомендации по патчингу
Попытается автоматически исправить простые уязвимости
Импортирует результаты в DefectDojo (если настроен API ключ)
